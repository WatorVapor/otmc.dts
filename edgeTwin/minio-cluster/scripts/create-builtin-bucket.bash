...
docker exec -it dts-minio /bin/bash -c "echo '{\"Version\": \"2012-10-17\",\"Statement\": [{\"Effect\": \"Allow\",\"Action\": [\"s3:*\"],\"Resource\": [\"arn:aws:s3:::dts-system-setting\",\"arn:aws:s3:::dts-system-setting/*\"]}]}' > /tmp/dts-repo-rw.json && mc policy set dts-builtin dts-builtin-system-admin /tmp/dts-repo-rw.json"
...
docker exec -it dts-minio /bin/bash -c "echo '{\"Version\": \"2012-10-17\",\"Statement\": [{\"Effect\": \"Allow\",\"Action\": [\"s3:*\"],\"Resource\": [\"arn:aws:s3:::dts-docker-repository\",\"arn:aws:s3:::dts-docker-repository/*\"]}]}' > /tmp/dts-repo-rw.json && mc policy set dts-builtin dts-builtin-mirror /tmp/dts-repo-rw.json"
