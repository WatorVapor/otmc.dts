...
const issueClientCertCluster = (req,res) => {
    readCsrPem(req,csrPem => {
        issueCSRCluster(csrPem,res);
    });
}
const issueCSRCluster = (csrPem,res) => {
    console.log('issueCSRCluster::csrPem:=<', csrPem, '>');
    console.log('issueCSRCluster::rootCAKeyPair:=<', rootCAKeyPair,'>');
    const outCert = signCSR(csrPem,validityYearsClient,rootCAKeyPair.privateKey,rootCAFilePath);
    console.log('issueCSRCluster::outCert:=<', outCert, '>');
    const response = {
